...
export default function App() {
  ...
  const handleSignup = async (name: string, email: string, pass: string, adminKey?: string) => {
    ...
    const adminEmail = ((import.meta as any).env?.VITE_ADMIN_EMAIL || 'admin@clickbazaar.com').toLowerCase();
    ...
    <>
      ...
      <div className="min-h-screen flex flex-col selection:bg-blue-600 selection:text-white bg-[#fafafa]">
        ...
        <nav className="sticky top-0 z-50 bg-white/80 backdrop-blur-xl border-b border-slate-200 no-print transition-all duration-300">
          <div className="max-w-7xl mx-auto px-4 lg:px-6 h-16 md:h-20 flex items-center justify-between">
            ...
            <div className="flex items-center gap-3 md:gap-4">
              <div className="hidden md:flex items-center gap-2 px-3 py-2 rounded-full bg-slate-100/60">
                <span className={`h-2 w-2 rounded-full ${backendOnline ? 'bg-emerald-500' : 'bg-rose-500'}`}></span>
                <span className="text-[10px] font-black uppercase tracking-widest text-slate-600">Backend: {backendOnline ? 'Online' : 'Offline'}</span>
                {!backendOnline && (
                  <button onClick={() => window.location.reload()} className="px-3 py-1 bg-blue-600 text-white rounded-full text-[10px] font-black uppercase tracking-widest">Retry</button>
                )}
              </div>
              <button
        ...
        {showBackendModal && !backendOnline && (
          <div className="fixed inset-0 z-[110] flex items-center justify-center bg-slate-950/80 backdrop-blur-lg p-4">
